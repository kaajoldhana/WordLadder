...
public class WordLadderTester {
	...
	public void testBFS3() {
		...
		Main.printLadder(res);
		String str = outContent.toString().replace("\n", "").replace("", "").trim();
		assertEquals("no word ladder can be found between stone and money", str);
	}
	@Test(timeout = 30000)
	public void testDFS4() {
		ArrayList<String> res = Main.getWordLadderDFS("anlas", "ogams");
		outContent.reset();
		Main.printLadder(res);
		String str = outContent.toString().replace("\n", "").replace("", "").trim();
		assertEquals("no word ladder can be found between anlas and ogams", str);
	}
